  ...
  return (dispatch, state) => {
    const checkFunctions = {
      ...
      age2: () => {
        ...
        deckCards.forEach((row, row_index) => {
          row.forEach((card, card_index) => {
            const cardMinusOneIndex = card_index - 1 < 0 ? null : card_index - 1;
            const nextUndefined = deckCards.get(row_index + 1) === undefined;
            const thisMinusOne = deckCards.getIn([row_index + 1, cardMinusOneIndex]);
      ...
      age3: () => {
        ...
        deckCards.forEach((row, row_index) => {
          row.forEach((card, card_index) => {
            ...
            const thisMinusOne = deckCards.getIn([row_index + 1, cardMinusOneIndex]);
            const thisEqual = deckCards.getIn([row_index + 1, card_index]);
            const thisPlusOne = deckCards.getIn([row_index + 1, card_index + 1]);
